...
constexpr char ssid[] = "kulit";
constexpr char password[] = "234567898";
constexpr char apiKey[] = "24878c4e2fa2c0f7fb2fd73f9a4017df38fe171f0f7aa418d87f7f92a1fe"; // Masukkan API key di sini

// ========================== Endpoint =============================
constexpr char authEndpoint[] = "https://sbd.srusun.id/control-agent/auth";
constexpr char controlEndpoint[] = "https://sbd.srusun.id/control-agent/recieve";
...
bool authenticateDevice() {
  ...
  //Melihat payload yang dikirim
  Serial.println("Payload auth:");
  Serial.println(body);

  int code = https.POST(body);

  if (code == 200) {
    String response = https.getString(); //ngambil string response dari server
    StaticJsonDocument<512> resDoc;
    deserializeJson(resDoc, response);// decode json response
    deviceToken = resDoc["data"]["token"].as<String>(); //ngambil token dan disimpan ke variabel device token
    https.end();
    return true;
  }

  Serial.printf("Auth failed, status code: %d\n", code);
  https.end();
  return false;
}
...
void sendDataToServer() {
  ...
  doc["device_mac_address"] = WiFi.macAddress();
  doc["device_token"] = deviceToken;  // ini bisa disimpan, meskipun tidak selalu dipakai oleh server
  doc["unit_id"] = unitId;
  doc["flat_id"] = flatId;
